import { showMessage, showNotifiy } from '../..';
...
export default [
  {
    ...
    walletNotInstallReducer: () => {
      return showNotifiy({
        message: 'Error: BlockWallet has not been installed.',
        className: 'installWalletTips',
        duration: 3000,
        description:'If you already have BlockWallet installed, check your browser extension settings to make sure you have it enabled and that you have disabled any other browser extension wallets.',
      },'warning')
    },
  },
  {
    walletType: COINBASE,
    icon: COINBASE,
    walletIsInstalledInvestigator: (provider:any) => provider.isCoinbaseWallet,
  },
  {
    walletType: BRAVE,
    icon: BRAVE,
    walletIsInstalledInvestigator: (provider:any) => isBraveWallet,
    chainIdTransfer: (chainId: string) => parseInt(chainId, 16),
    walletNotInstallReducer: () => {
  ...
  },
  {
    walletType: TALLYHO,
    icon: TALLYHO,
    walletIsInstalledInvestigator: (provider:any) => provider.isTally,
    walletNotInstallReducer: () => {
      showMessage(
  ...
  },
  {
    ...
    walletNotInstallReducer: () => {
      return showNotifiy({
        message: 'Error: OKXWallet has not been installed.',
        // dangerouslyUseHTMLString: true,
        // type: 'warning',
        className: 'installWalletTips',
        duration: 3000,
        description:
            'If you already have OKXWallet installed, check your browser extension settings to make sure you have it enabled and that you have disabled any other browser extension wallets.',
      },'warning')
    },
  },
]
